...
وضعیت فعلی: Phase 0، Phase 1 (ورود واقعی و پوسته محافظت‌شده)، Phase 2 (ساختار سازمانی چهارسطحی)، Phase 3 (انواع خودرو، نوع بار و ناوگان)، Phase 4 (نقشه داخلی و نمایش دفاتر/انبارها)، Phase 5 (مدیریت مسیر، CSV و ترسیم روی نقشه)، Phase 6 (تعریف مرسوله و مقصد)، Phase 7 (برنامه‌ریزی مأموریت از فرم)، Phase 8 (تعریف مأموریت از داخل نقشه) و Phase 9 (موتور موقعیت تقریبی — لایه محاسباتی pure بدون UI، طبق `docs/phase-09-simulation-engine/`) تکمیل شده‌اند.
...
- `GET /api/v1/missions/:id/simulate?viewTime=...` — موتور محاسبه موقعیت تقریبی (فاز ۹)؛ صفحه UI ندارد، فقط API فقط-خواندنی برای مصرف فاز ۱۰ به بعد؛ برای هر سه نقش احرازهویت‌شده باز است. جزئیات کامل در `docs/phase-09-simulation-engine/`.
