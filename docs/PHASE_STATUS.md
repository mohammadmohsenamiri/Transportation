...
| 14 | مدیریت کاربران، آیکن‌ها و تنظیمات تکمیلی | کاربران، نقش‌ها، آیکن سفارشی و audit viewer | DONE | M | Development Pack در `docs/phase-14-users-icons-settings/`؛ ADR-030 |
| 15 | تکمیل چرخه عمر مأموریت | وضعیت پایانی واقعی، زمان واقعی و کنترل همروندی | DONE | M | Development Pack در `docs/phase-15-mission-lifecycle-completion/`؛ ADR-031 |
...
### Phase 14 — مدیریت کاربران، آیکن‌های نقشه و تنظیمات سامانه

Status: DONE
Started: 2026-08-08
Completed: 2026-08-08
Visible output URL: چهار صفحه جدید زیر `/system` (فقط Admin): `/system/users` (فهرست صفحه‌بندی‌شده با جست‌وجو و پالایه نقش/وضعیت، ساخت کاربر، ویرایش نام نمایشی و نقش‌ها، فعال/غیرفعال، تعلیق با دلیل، رفع تعلیق، بازنشانی رمز، حذف نرم و بازگردانی)، `/system/icons` (کتابخانه آیکن با بارگذاری PNG/SVG، پیش‌نمایش، جایگزینی فایل، حذف نرم و بازگردانی)، `/system/settings` (۱۳ تنظیم گروه‌بندی‌شده با ذخیره دسته‌ای، بازگرداندن به پیش‌فرض و نشان‌دادن کلیدهای قفل‌شده با متغیر محیطی) و `/system/audit` (گزارش تغییرات با پالایه کنش/موجودیت، زمان شمسی و نمایش before/after). چهار تب غیرفعال نوار `/system` فعال شدند. روی نقشه عملیاتی، خودروهای دارای آیکن تعیین‌شده به‌جای دایره رنگی با آیکن سفارشی و حلقه رنگی وضعیت رسم می‌شوند.
Demo account/data: کاربران Admin/Planner/Viewer از فازهای قبل. برای دموی آیکن یک PNG بین ۱۶ تا ۵۱۲ پیکسل یا یک SVG ساده کافی است.
Branch/PR/Commit: مستقیم روی `main`
Migrations: `20260808070005_phase14_users_icons_settings` — **کاملاً افزایشی، بدون backfill**. به `User`: `displayName`, `deletedAt`, `lastLoginAt`, `suspendedAt`, `suspensionReason`, `version` + دو ایندکس. جدول‌های جدید `IconAsset` و `SystemSetting` (کلید اصلی `key`). enum جدید `IconCategory`. ستون nullable `iconAssetId` روی `OrganizationUnit`، `VehicleType` و `Vehicle` با `onDelete: SetNull` و ایندکس. هیچ ستون موجودی تغییر نکرد یا حذف نشد، پس rollback فقط با حذف افزوده‌ها ممکن است.
Key files: **لایه محض:** `src/lib/domain/user-rules.ts` (اشتقاق وضعیت، اعتبارسنجی نام کاربری/نام نمایشی/رمز/دلیل، قاعده «آخرین مدیر»)، `src/lib/domain/svg-analyzer.ts` (تحلیل‌گر allowlist)، `src/lib/domain/icon-rules.ts` (اعتبارسنجی فایل با بررسی magic bytes و خواندن ابعاد PNG از IHDR بدون هیچ dependency، و `resolveIcon`)، `src/lib/settings/settings-registry.ts` (۱۳ تعریف تنظیم + `assertRegistryIsSound`). **سرویس:** `src/server/services/user-service.ts`، `icon-service.ts`، `icon-storage.ts`، `settings-service.ts`، `audit-query-service.ts`؛ `auth-service.ts` اصلاح شد تا کاربر معلق/حذف‌شده را با پیام یکسان رد کند و `lastLoginAt` را بدون دست‌زدن به `version` بنویسد. **API:** ۱۹ route زیر `src/app/api/v1/{users,icons,settings,audit}` + `src/lib/validation/admin.ts`، `src/lib/http/admin-errors.ts` (نگاشت واحد کد خطا→HTTP و سهمیه نوشتن)، `src/lib/http/user-transition-route.ts`، `src/lib/http/icon-multipart.ts`. **UI:** `src/features/{users,icons,settings,audit}/*` و چهار `page.tsx`. **نقشه:** `map-scene-service.ts` (یک پرس‌وجوی ثابت اضافه برای آیکن‌های قابل استفاده، نه N تا) و `maplibre-map-inner.tsx` (رندر آیکن با بازگشت بی‌صدا به دایره).
Tests executed: `npm run typecheck` و `npm run lint` (بدون خطا؛ همان warningهای `react-hook-form` که کل کدبیس دارد)، `npm run test` (**۳۲۵ تست Vitest**، شامل ۹۶ تست جدید: ۲۳ برای قواعد کاربر، ۵۸ برای آیکن/SVG شامل **۱۸ فیکسچر SVG خصمانه که همگی رد می‌شوند** و نمونه‌های بی‌خطر که پذیرفته می‌شوند، ۱۵ برای رجیستری تنظیمات)، `npm run build` (موفق؛ هر ۱۹ route و ۴ صفحه ثبت شدند — این گام واقعاً لازم است چون نقض مرز client/server را می‌گیرد که `tsc` نمی‌گیرد)، و `npx playwright test admin-users-icons-settings` (**۱۴۴ تست = ۳۶ سناریو × ۴ viewport، همه سبز**).
Regression suite result: اجرای کامل `npx playwright test` → **۴۵۸ موفق، ۲۴ شکست، ۲ skip**. هیچ‌کدام از ۲۴ شکست رگرسیون این فاز نیست؛ همگی با اجرای ایزوله همان specها اثبات شدند: `map.spec.ts` + `map-scene.spec.ts` → **۴۰/۴۰ سبز**، و `situation-dashboard.spec.ts` + `timeline-engine.spec.ts` + `auth.spec.ts` → **۱۱۶/۱۱۶ سبز** (یعنی R-01 هم برقرار است). `fleet.spec.ts:18` و `organization.spec.ts:18` همان دو شکست پیش‌موجودِ مستندشده در ممیزی فاز ۱۳ هستند (نبود pagination در حجم فعلی DB). افزودن یک spec بزرگ جدید فشار موازی‌سازی را بالا برده و تست‌های delta-محور را زیر بار حساس‌تر کرده است.
Manual demo steps: ورود Admin → `/system/users` → «کاربر جدید» با نقش «برنامه‌ریز مأموریت» → خروج و ورود با آن کاربر → اجبار به تغییر رمز → بازگشت به Admin → تعلیق همان کاربر با دلیل → تلاش ورود آن کاربر و مشاهده رد شدن → رفع تعلیق → `/system/icons` → بارگذاری یک PNG و مشاهده بندانگشتی → تلاش بارگذاری یک SVG حاوی `<script>` و مشاهده پیام فارسی که **نام سازه ناامن را می‌برد** → تخصیص آیکن به یک خودرو و مشاهده تغییر نشانگر در `/map` → حذف نرم آیکن و مشاهده بازگشت بی‌صدای نشانگر به دایره (بدون خطا و بدون تصویر شکسته) → `/system/settings` → تغییر یک تنظیم، ذخیره، بازگرداندن به پیش‌فرض → `/system/audit` → پالایش روی «کاربر» و دیدن رد همه اقدامات بالا با زمان شمسی.
Offline/network verification: هیچ dependency جدیدی نصب نشد و هیچ درخواست بیرونی اضافه نشد؛ تست `OF-02` روی هر چهار viewport اثبات می‌کند چهار صفحه مدیریتی **صفر** درخواست غیر-localhost می‌فرستند. بایت‌های آیکن از همان origin و از یک route احرازهویت‌شده سرو می‌شوند، نه از CDN یا `public/`.
Security posture: هر ۱۹ endpoint مدیریتی گیت `requireActor([RoleCode.ADMIN])` را **پیش از خواندن بدنه** اجرا می‌کنند و تست‌های SEC-01/02/03 این را برای هر سه حالت (بدون ورود، برنامه‌ریز، بیننده) روی *تک‌تک* endpointها assert می‌کنند — پنهان‌کردن دکمه در UI هیچ‌جا به‌عنوان کنترل حساب نشده است. مجوز بر وجود رکورد مقدم است (۴۰۳ نه ۴۰۴). رمز و هش رمز در هیچ پاسخ API و هیچ رکورد audit ظاهر نمی‌شوند (`auditSafe` با allowlist فیلد کار می‌کند، نه با حذف فیلدهای شناخته‌شده). ایمنی SVG لایه‌ای است: رندر فقط با `<img src>` از endpointی با `Content-Security-Policy: default-src 'none'; sandbox` و `X-Content-Type-Options: nosniff`، و تحلیل‌گر allowlist به‌عنوان لایه دوم. مسیرپیمایی در نام فایل و در URL محتوا هر دو تست شده‌اند. سهمیه نوشتن مدیریتی (۱۲۰ در دقیقه به‌ازای هر actor) اضافه شد.
Known limitations: **سهمیه نرخ و cache تنظیمات per-process هستند** — مثل محدودکننده نرخ ورود که از فاز ۱ همین‌طور بوده؛ در استقرار چندنمونه‌ای باید به store مشترک منتقل شوند (فاز ۱۷). **تخصیص آیکن توکن نسخه نمی‌گیرد** و آخرین نوشتن برنده است، چون سه موجودیت مقصد ستون `version` ندارند؛ انحراف عمدی از §۴٫۶ سند API فاز، ثبت‌شده در ADR-030 §۹. **آیکن سفارشی با bearing نمی‌چرخد** (ADR-030 §۸). **گذارهای وضعیت از UI صفحه فهرست انجام می‌شوند و صفحه جزئیات کاربر ساخته نشد**؛ دلیل تعلیق در سیاهه ممیزی و از راه API قابل مشاهده است ولی صفحه اختصاصی ندارد. **سناریوی سراسری «آخرین مدیر» در e2e بازسازی نمی‌شود** (ADR-030 §۱۰) — مرز قاعده در تست واحد و همروندی آن در CX-02 پوشش دارد. `SystemSetting`ها هنوز روی رفتار زمان اجرا اثر نمی‌گذارند مگر جایی که سرویس مربوطه آن‌ها را بخواند؛ سیم‌کشی کامل مصرف‌کننده‌ها به فازهای بعد موکول است.
Deferred items: صفحه جزئیات/پروفایل کاربر، مدیریت نشست‌های فعال از UI، خروجی CSV سیاهه ممیزی، صفحه تخصیص گروهی آیکن به واحدهای سازمانی، همگام‌سازی cache تنظیمات بین نمونه‌ها (فاز ۱۷).
Decisions added/changed: ADR-030 اضافه شد (ده تصمیم: حفظ مدل نقش‌محور بدون جدول Permission، ایمنی لایه‌ای SVG، ذخیره فایل بیرون از `public/`، رجیستری کد به‌جای ستون نوع، ترتیب و مرتب‌سازی قفل «آخرین مدیر»، نگاشت تعارض ساخت هم‌زمان تنظیم به ۴۰۹، سهمیه نوشتن مدیریتی، عدم چرخش آیکن سفارشی، انحراف مستند تخصیص آیکن از سند API، و راهبرد آزمون «آخرین مدیر»). `docs/ARCHITECTURE_AND_DATA_MODEL.md` با `SystemSetting`، ستون‌های تازه `User`، جزئیات `IconAsset`، enum جدید `IconCategory` و این نکته که دسته آیکن قاعده تعیین را محدود نمی‌کند، به‌روزرسانی شد.

**نقص‌های واقعی که حین آزمون این فاز کشف و رفع شدند** (هر سه در کد محصول بودند، نه در تست):

1. `SELECT count(*) … FOR UPDATE` در PostgreSQL نامعتبر است (`0A000`) و گیت «آخرین مدیر» را کاملاً می‌شکست؛ شمارش به JS منتقل شد.
2. قفل‌گذاری *پس از* به‌روزرسانی، دو غیرفعال‌سازی موازی را به بن‌بست می‌برد و به‌جای ۴۰۹ خطای ۵۰۰ می‌داد؛ قفل به ابتدای تراکنش و با `ORDER BY u.id` منتقل شد.
3. ساخت هم‌زمان یک تنظیم بدون رکورد، نقض کلید یکتای خام (۵۰۰) می‌داد؛ به `SETTING_VERSION_CONFLICT` (۴۰۹) نگاشت شد.

**نقص‌های عرضی که همین فاز آشکار کرد و رفع شدند:** پیام خطای ورود `role="alert"` نداشت و برای کاربر screen reader بی‌صدا بود؛ و ستون محتوای پوسته برنامه `min-w-0` نداشت، پس هر محتوای عریض (جدول یا نوار تب) به‌جای اسکرول درون ظرف خودش، کل صفحه را افقی اسکرول می‌کرد — با افزوده‌شدن تب پنجم `/system` این دقیقاً رخ داد و اکنون هر چهار صفحه مدیریتی روی هر چهار عرض اسکرول افقی صفحه ندارند. `tests/e2e/seed-users.ts` هم اصلاح شد تا ستون‌های وضعیت جدید را بازنشانی کند و خط پایه e2e قطعی بماند.
...
Started: 2026-08-08
Completed: 2026-08-08
Visible output URL: صفحه جزئیات هر مأموریت (`/missions/{id}`) اکنون پنل «چرخه عمر مأموریت» دارد: ثبت تکمیل (با زمان رسیدن واقعی و زمان حرکت واقعی اختیاری)، ثبت شکست (با طبقه‌بندی و شرح علت)، بازگشایی، بایگانی و خروج از بایگانی — هر کدام با دیالوگ تأیید و ورودی‌های لازم. زمان‌های واقعی *در کنار* زمان‌های برنامه‌ریزی‌شده و با برچسب متمایز نمایش داده می‌شوند، به‌همراه اختلاف رسیدن از تخمین به دقیقه. رشته یادداشت زمان‌دار و دارای نویسنده به همان صفحه افزوده شد. صفحه مدیریتی تازه `/system/mission-types`. برچسب «رسیده» در کل سامانه به **«رسیده (تخمینی)»** تغییر کرد و دو وضعیت تازه «تکمیل‌شده» و «ناموفق» به فهرست مأموریت‌ها، جزئیات، جدول عملیاتی و شمارنده‌های خلاصه اضافه شدند.
Demo account/data: کاربران Admin/Planner/Viewer از فازهای قبل. برای دمو یک مأموریت منتشرشده لازم است که زمان شروعش گذشته باشد.
Branch/PR/Commit: مستقیم روی `main`
Migrations: `20260808161000_phase15_mission_lifecycle` — **کاملاً افزایشی، بدون backfill**. مقادیر `COMPLETED` و `FAILED` به enum `MissionPersistedStatus` افزوده شدند (`ARCHIVED` از فاز ۷ وجود داشت و اینجا برای نخستین بار *نوشته* می‌شود). enum جدید `MissionFailureClassification`. ستون‌های تازه `Mission`: `actualDepartureAt`, `actualArrivalAt`, `failedAt`, `failureReason`, `failureClassification`, `archivedAt`, `statusBeforeArchive`, `reopenCount`, `lastReopenedAt`, `version`, `missionTypeId` — همه nullable یا دارای پیش‌فرض. جدول‌های جدید `MissionType` و `MissionNote`. سه ایندکس تازه. **PostgreSQL نسخه ۱۲٫۳ این محیط بررسی شد**: از PG 12 افزودن مقدار enum داخل تراکنش مجاز است تا وقتی همان مقدار در همان تراکنش *استفاده* نشود؛ این migration فقط مقدار را اضافه می‌کند، پس نیازی به تقسیم به دو migration نبود. ⚠️ **این یک در یک‌طرفه است**: پس از تکمیل نخستین مأموریت، بازگشت به عقب یعنی از دست دادن یک واقعیت کسب‌وکاری، و مقدار enum در PostgreSQL اصلاً قابل حذف نیست.
Key files: **لایه محض:** `src/lib/domain/mission-lifecycle.ts` (جدول گذارها به‌صورت داده + گاردهای LR-01…LR-14 + `arrivalVarianceMinutes`)، `mission-rules.ts` (دو شرط کوتاه‌کننده تازه در `deriveMissionDisplayStatus` و `isTerminalPersistedStatus`)، `mission-simulation.ts` (انجماد `COMPLETED`/`FAILED`)، `mission-labels.ts` (برچسب‌های تازه + تغییر برچسب `ARRIVED`). **سرویس:** `mission-service.ts` (پنج گذار تازه + `updateMissionGuarded` + نسخه روی `updateMission`/`cancelMission`/`commitMissionAssignment` + دو سطل تازه در خلاصه)، `mission-type-service.ts`، `mission-note-service.ts`. **API:** ده route تازه زیر `missions/[id]/{complete,fail,archive,unarchive,reopen,notes}` و `mission-types` + `src/lib/validation/mission-lifecycle.ts` + `src/lib/http/{mission-errors,mission-transition-route}.ts`. **UI:** `mission-lifecycle-panel.tsx`، `mission-note-thread.tsx`، `mission-type-list.tsx`، و افزوده‌های `mission-detail-view.tsx`. **کلاینت:** `ApiError` اکنون `code` را نگه می‌دارد.
Tests executed: `npm run typecheck` و `npm run lint` (بدون خطا؛ همان warningهای `react-hook-form` که کل کدبیس دارد)، `npm test` (**۳۶۱ تست Vitest** شامل ۳۶ تست تازه `mission-lifecycle.test.ts`: بررسی *جامع* هر جفت (وضعیت، عملیات) در جدول گذارها، هر مرز زمانی (E1/E2/E3)، پذیرش صریح دیرکرد و زودکرد (LR-03)، اولویت وضعیت ثبت‌شده بر ساعت در هر لحظه (E8/E12)، و هر دو قاعده انجماد؛ به‌علاوه به‌روزرسانی تست `missionCancelSchema` برای قرارداد تازه)، `npm run build` (موفق، ۵۴ صفحه)، و `npx playwright test mission-lifecycle` روی چهار viewport.
Regression suite result: **پیش از نوشتن هر تست e2e**، کل چرخه با `scripts/verify-lifecycle.ts` روی دیتابیس واقعی اجرا شد و این‌ها اثبات شدند: گذار نامعتبر ⇒ `MISSION_INVALID_TRANSITION`، توکن کهنه روی گذارِ مجاز ⇒ `MISSION_VERSION_CONFLICT`، بازگشت دقیق `statusBeforeArchive`، پاک شدن واقعیت‌های پایانی هنگام بازگشایی، و رسیدن مرسوله به `WAITING_FOR_DISPATCH` پس از شکست. نکته مهم: **گذر از تست واحد کافی نبود** — سه نقص واقعی فقط در اجرای روی DB و در آزمون همروندی ظاهر شدند (پایین).
Manual demo steps: ورود Admin → ساخت مأموریت با زمان شروع چند دقیقه بعد و انتشار آن → صبر تا گذشتن زمان شروع → `/missions/{id}` → مشاهده «رسیده (تخمینی)» → «ثبت تکمیل» با زمان رسیدن واقعی → مشاهده تغییر برچسب به «تکمیل‌شده» و نمایش اختلاف با تخمین → مشاهده خروج مأموریت از `/map` و جابه‌جایی شمارنده در `/dashboard` → «بازگشایی» با دلیل و مشاهده بازگشت به «برنامه‌ریزی‌شده» با پاک شدن زمان رسیدن → «ثبت شکست» با طبقه‌بندی → «بایگانی» و «خروج از بایگانی» → افزودن یادداشت → `/system/mission-types` و ساخت یک نوع.
Offline/network verification: هیچ dependency تازه‌ای نصب نشد. تست `OF-01` اثبات می‌کند صفحه جزئیات مأموریت جز کاشی‌های Provider نقشه هیچ درخواست بیرونی نمی‌فرستد — و آن کاشی‌ها هم انتخاب پیکربندی فاز ۴ در این محیط‌اند (Provider فعال این محیط توسعه، OSM خارجی است)، نه چیزی که فاز ۱۵ آورده باشد.
Known limitations: **تخصیص نوع مأموریت به خود مأموریت در ویزارد سیم‌کشی نشد** — ستون `missionTypeId`، سرویس، API و صفحه مدیریتی کامل‌اند و DTO آن را برمی‌گرداند، ولی انتخاب نوع در فرم ساخت/ویرایش مأموریت به فاز بعد موکول شد؛ در نتیجه امروز فقط از راه API قابل تنظیم است. **مأموریت فقط پس از فرارسیدن زمان شروع برنامه‌ریزی‌شده‌اش قابل تکمیل است** (ADR-031 §۵) — رفتار درست، ولی چون در سند فاز صریح نبود اینجا ثبت شد. **`COMPLETED`/`FAILED` در پالایه نقشه عملیاتی نیستند** (ADR-031 §۴) چون صحنه نقشه فقط `SCHEDULED` را می‌خواند. **ورودی تاریخ/ساعت رابط کاربری دقت دقیقه‌ای دارد**، پس مأموریتی که چند ثانیه پیش و در همان دقیقه جاری شروع شده، از راه UI بلافاصله قابل تکمیل نیست: مقدار پیش‌فرضِ گردشده به ابتدای دقیقه، پیش از زمان شروع می‌افتد و LR-02 آن را رد می‌کند. در عمل بی‌اثر است (مأموریت‌های واقعی ساعت‌ها طول می‌کشند) ولی یک لبه واقعی است و اینجا ثبت می‌شود؛ از راه API با دقت ثانیه محدودیتی وجود ندارد. **`Mission.notes` قدیمی و رشته یادداشت تازه هر دو نمایش داده می‌شوند** (ADR-P15-09). بندهای `CX-03/05/06` و `E-14/15/16` سند آزمون پوشش خودکار ندارند.
Deferred items: انتخاب نوع مأموریت در ویزارد، گزارش دقت تخمین بر پایه `arrivalVarianceMinutes`، چندتوقفی/کاروان/تکرارشونده و جریان تأیید (همگی نقاط توسعه طراحی‌شده در §۳ سند `01-SCOPE.md`).
Decisions added/changed: ADR-031 اضافه شد (شش تصمیم پیاده‌سازی: ارزیابی گارد درون تراکنش به‌جای تکیه بر توکن نسخه، خواندن قفل‌دار در مسیر عملیاتی `updateMission` برای حفظ «دقیقاً یک واحد» بودن افزایش نسخه، نگه‌داشتن `code` در `ApiError` سمت کلاینت، عدم افزودن دو وضعیت تازه به پالایه نقشه، ثبت قید زمانی برخاسته از ترکیب فاز ۷ و ۱۵، و نمایش جداگانه یادداشت قدیمی و رشته تازه).

**سه نقص واقعی که آزمون روی DB و آزمون همروندی آشکار کرد** (هر سه در کد محصول، نه در تست):

1. **گارد «آخرین مدیر» نبود، ولی الگوی مشابهی بود:** `updateMissionGuarded` نخست با `Prisma.MissionUpdateManyMutationInput` نوشته شد که کلیدهای خارجی اسکالر مثل `updatedById` را نمی‌پذیرد؛ typecheck آن را گرفت و به `Unchecked…` تغییر کرد.
2. **گارد گذار بیرون از تراکنش ارزیابی می‌شد** و درستی‌اش به این وابسته بود که هر مسیر نوشتنی نسخه را بالا ببرد — و `publishMission` این کار را نمی‌کرد. هر دو اصلاح شدند (ADR-031 §۱).
3. **افزودن نوشتن مشروط در ابتدای `commitMissionAssignment` نسخه را دو واحد جابه‌جا می‌کرد** و CC-01 را می‌شکست؛ با خواندن قفل‌دار جایگزین شد (ADR-031 §۲).

**نکته‌ای درباره تست‌ها که ارزش ثبت دارد:** fixture این فاز ناچار است منتظر گذشتن زمان تخمینی رسیدن بماند و خودرو را **تصادفی** از میان خودروهای آماده انتخاب کند — چون فاز ۷ دو مأموریت هم‌پوشان روی یک خودرو را رد می‌کند و چهار پروژه viewport به‌موازات اجرا می‌شوند. نسخه اول که همیشه اولین خودرو را برمی‌داشت، در اجرای موازی خودش را می‌شکست.
