...
| Release Candidate | 18 | نسخه آماده UAT و استقرار شبکه داخلی |
...
> **این فاز یک Development Pack کامل دارد:** `docs/phase-09-simulation-engine/` (۱۶ سند: دامنه، الزامات، معماری، الگوریتم، API، تست، پذیرش و ADR). قبل از پیاده‌سازی این فاز، `docs/phase-09-simulation-engine/00-README.md` و به‌ترتیب اسناد بعدی خوانده شوند؛ آن پک مرجع الزام‌آور جزئیات این فاز است و این بخش فقط خلاصه هدف/دامنه/معیار پذیرش را نگه می‌دارد.

### هدف

پیاده‌سازی منطق اصلی محاسبه موقعیت تقریبی به‌صورت یک لایه محاسباتی pure، مستقل از UI/React/Next.js/کتابخانه نقشه، پیش از ادغام آن با نقشه عملیاتی در فاز ۱۰.

### دامنه پیاده‌سازی

- تابع pure و deterministic برای محاسبه:
  - وضعیت مأموریت در زمان انتخاب‌شده (با استفاده مجدد از `deriveMissionDisplayStatus` فاز ۷، بدون بازنویسی)؛
  - فاصله طی‌شده و باقی‌مانده؛
  - درصد پیشرفت؛
  - مختصات تقریبی؛
  - ETA و زمان باقی‌مانده؛
  - bearing تقریبی.
- interpolation روی RoutePointهای تجمعی (binary search + interpolation خطی lat/lng).
- fallback حرکت مستقیم بین مبدأ و مقصد برای مأموریت فاقد مسیر (طبق ADR-011).
- یک endpoint فقط-خواندنی داخلی (`GET /api/v1/missions/:id/simulate`) برای مصرف توسط فاز ۱۰ و آینده.
- تست‌های مرزی گسترده (جزئیات کامل در `docs/phase-09-simulation-engine/08-TESTS.md`).

**تغییر آگاهانه نسبت به نسخه قبلی این بخش:** صفحه داخلی `/system/simulation-lab` از دامنه این فاز حذف شد — طبق دستور صریح مالک محصول، فاز ۹ نباید هیچ کد React/Next.js UI یا کتابخانه نقشه داشته باشد. جزئیات کامل تصمیم در ADR-024 (`docs/DECISIONS.md`) و `docs/phase-09-simulation-engine/ADR.md` (ADR-P9-01) ثبت شده است؛ صفحه lab (در صورت نیاز) به فاز ۱۰ موکول شده است.

### خروجی قابل مشاهده

از آنجا که این فاز بدون UI است، «خروجی قابل مشاهده» آن مطابق تعریف در `docs/phase-09-simulation-engine/09-ACCEPTANCE.md` بازتعریف شده: مجموعه تست خودکار سبز و قطعی، به‌همراه یک اجرای دستی اسکریپتی (غیرقابل‌کامیت) که چهار لحظه از یک مأموریت نمونه (پیش از شروع، میانه مسیر، لحظه ETA، پس از لغو) را چاپ می‌کند.

### سناریوی نمایش

بررسی یک مأموریت قبل از شروع، در میانه مسیر، لحظه ETA و پس از رسیدن؛ سپس تکرار با مأموریت بدون مسیر و یک مأموریت لغوشده. جزئیات دقیق در `docs/phase-09-simulation-engine/09-ACCEPTANCE.md` بخش ۳.

### معیار پذیرش

نتایج برای ورودی یکسان همیشه یکسان باشند، موقعیت هر tick در DB ذخیره نشود، و تمام معیارهای `docs/phase-09-simulation-engine/09-ACCEPTANCE.md` برآورده شوند.
...
بستن شکاف «سامانه فقط می‌داند چه چیزی *برنامه‌ریزی* شده و ساعت چه می‌گوید، ولی نمی‌داند واقعاً چه اتفاقی افتاد». فازهای ۷ و ۸ ساخت، انتشار، ویرایش، لغو و تکثیر مأموریت را شیپ کرده‌اند؛ این فاز آن‌ها را با وضعیت‌های پایانی واقعی، زمان‌های واقعی و کنترل همروندی کامل می‌کند. **هیچ بخشی از سطح موجود فاز ۷/۸ بازنویسی نمی‌شود.**

### دامنه پیاده‌سازی

- وضعیت‌های پایانی نوشتنی: تکمیل (با تأیید اپراتور و زمان واقعی رسیدن)، شکست (با دلیل و طبقه‌بندی)، بایگانی (مقدار `ARCHIVED` امروز در enum هست ولی هرگز نوشته نمی‌شود)، و بازگشایی.
- زمان‌های *واقعی* در کنار زمان‌های برنامه‌ریزی‌شده (`actualStartAt`، `actualArrivalAt`، `failedAt`) و قاعده صریح رفتار وقتی با تخمین اختلاف دارند.
- گسترش `deriveMissionDisplayStatus` به‌گونه‌ای که وضعیت پایانیِ ثبت‌شده بر وضعیت مشتق‌شده از ساعت اولویت بگیرد — دقیقاً همان الگویی که امروز برای `CANCELLED` وجود دارد. این تضمین ADR-029 (یکسان‌بودن عدد داشبورد و نقشه) را بدون هزینه حفظ می‌کند.
- کنترل همروندی خوش‌بینانه روی خود مأموریت (ستون `version`) تا دو ویرایش هم‌زمان بی‌صدا یکدیگر را بازنویسی نکنند. قفل بدبینانه مرسوله (ADR-019) دست‌نخورده می‌ماند؛ این دو مسئله متفاوت را حل می‌کنند.
- `MissionType` به‌عنوان داده مرجع (بدون hardcode در business logic، طبق `CLAUDE.md` §۵).
- یادداشت مأموریت به‌صورت نخ زمان‌دار به‌جای فیلد اسکالر فعلی.
- audit و history برای هر گذار جدید، هم‌سطح عملیات موجود.

### خروجی قابل مشاهده

اپراتور رسیدن یک مأموریت را تأیید می‌کند یا آن را با دلیل شکست‌خورده اعلام می‌کند؛ عدد داشبورد و نقشه هم‌زمان و سازگار تغییر می‌کنند و تاریخچه، گذار را با کاربر و زمان ثبت می‌کند.

### سناریوی نمایش

انتشار یک مأموریت، تأیید تکمیل با زمان واقعیِ متفاوت از ETA، سپس شکست‌دادن یک مأموریت دیگر و بایگانی یک مأموریت قدیمی؛ و تلاش دو ویرایش هم‌زمان که دومی با خطای همروندی رد شود.

### معیار پذیرش

گذارهای نامعتبر از سرویس رد شوند، وضعیت پایانی ثبت‌شده همیشه بر وضعیت ساعت‌محور غلبه کند، و ویرایش هم‌زمان هرگز بی‌صدا بازنویسی نشود.

---

## Phase 16 — ریسپانسیو، Touch و دسترس‌پذیری نهایی
...
## Phase 17 — اجرای بدون اینترنت، امنیت، عملیات و ظرفیت
...
## Phase 18 — UAT و Release Candidate
...
- هدف اولیه p95 برای queryهای list/dashboard روی dataset آزمون کمتر از 500ms و map scene کمتر از 1s است؛ نتیجه واقعی در Phase 17 اندازه‌گیری و ثبت شود.
...
# پیوست — قابلیت پیشنهادی آتی: درون‌ریزی سبک نقشه Mapnik XML

این پیوست بخشی از فازهای متعهدشده ۰ تا ۱۸ بالا **نیست** و طبق قرارداد اجرای فاز (بخش «قرارداد اجرای فاز توسط Claude» در همین سند) و ممنوعیت صریح `CLAUDE.md` («خارج از فاز جاری feature آینده را scaffold نکن») نباید بدون تخصیص یک فاز مستقل و تأیید صریح مالک محصول کدنویسی یا migration شود. هدف این پیوست فقط ثبت این است که **اگر** این قابلیت در آینده به یکی از فازها اضافه یا به‌عنوان فاز جدید (مثلاً Phase 19) افزوده شود، معماری آن از پیش تصمیم‌گیری‌شده باشد.

## خلاصه قابلیت

Admin بتواند یک فایل سبک Mapnik (پسوند `.xml`) را از محیط لوکال خودش بارگذاری کند تا به یک Provider نقشه داخلی جدید (کاشی XYZ/TMS قابل مصرف توسط MapLibre GL همین سامانه) تبدیل شود.

## چرا این یک فاز جداست، نه بخشی از فاز ۴

Mapnik XML یک فرمت رندر سمت‌سرور است، نه یک سبک قابل‌مصرف مستقیم توسط MapLibre GL JS (رندرر سمت‌کاربر همین سامانه) — بین آپلود فایل و دیدن نتیجه روی نقشه، یک مرحله رندر واقعی (نیازمند موتور Mapnik، یک وابستگی بومی/سیستمی سنگین و کاملاً اختیاری) یا یک workflow پیش‌رندر آفلاین لازم است. این یعنی حجم کار واقعی (job رندر پس‌زمینه، sandbox امنیتی XML، مدل داده جدید `MapStyleSource`) به‌مراتب بزرگ‌تر از «یک قابلیت اصلی کسب‌وکاری» تعریف‌شده برای هر فاز است و طبق قاعده «اگر فاز در عمل بزرگ‌تر شد، به زیرفاز A/B تقسیم شود» باید یک فاز (یا زیرفاز) مستقل خودش باشد.

## مرجع معماری

جزئیات کامل معماری، مدل داده پیشنهادی (`MapStyleSource`) و الزامات امنیتی (sandbox مسیر فایل، غیرفعال‌سازی XXE، محدودیت نوع Datasource، سقف حجم/زمان) در **ADR-022** (`docs/DECISIONS.md`) و بخش «Mapnik Style (XML)» زیر «Upload security» در `docs/API_SECURITY_OFFLINE_OPERATIONS.md` ثبت شده است. هر فاز اجرایی آینده که این قابلیت را برمی‌دارد باید همان دو سند را به‌عنوان مرجع الزام‌آور در نظر بگیرد، نه این پیوست را بازنویسی کند.

## پیش‌نیاز حداقلی برای شروع پیاده‌سازی واقعی

- تصمیم مالک محصول درباره اینکه آیا مسیر «رندر سمت‌سرور» (نیازمند نصب Mapnik روی میزبان استقرار) در نسخه اول ساخته شود یا فقط مسیر ایمن‌تر «پیش‌رندر آفلاین توسط Admin/DevOps و آپلود فقط خروجی کاشی نهایی» کافی تلقی شود.
- تخصیص شماره فاز مشخص (یا زیرفاز ضمیمه به یکی از فازهای هنوز اجرانشده) در این سند، طبق فرمت استاندارد فازهای بالا (هدف/دامنه/خروجی قابل‌مشاهده/سناریوی نمایش/معیار پذیرش/خارج از دامنه).
