...
**تصمیم اضافه (کشف‌شده حین آزمون رگرسیون، نه یکی از پنج سؤال باز سند) — ADR-028-A:** `useMapScene` (فاز ۱۰) با تغییر `viewTime` یک `queryKey` کاملاً جدید می‌سازد (`["map","scene",viewTime ?? "live"]`)؛ بدون `placeholderData`، هنگام سوییچ Live→Historical یا تغییر لحظه تاریخی، `data` بین دو fetch لحظه‌ای `undefined` می‌شود. چون `map-view.tsx` این را با `sceneQuery.data?.missions ?? []` می‌خواند، آرایه مأموریت‌ها موقتاً خالی می‌شود و قاعده «پاک‌کردن انتخاب اگر مأموریت دیگر در لیست فیلترشده نیست» در `use-mission-interaction.ts` بلافاصله `selectedMissionId` را پاک می‌کند — یعنی هر جهش/تغییر زمان، انتخاب کاربر روی نقشه را از بین می‌برد. با افزودن `placeholderData: keepPreviousData` (`@tanstack/react-query` v5) به `useMapScene`، داده قبلی حین بارگذاری داده جدید نگه داشته می‌شود و این رگرسیون رفع شد؛ در تست e2e موبایل (`تست پرش-به-زمان`) به‌طور پایدار قابل بازتولید بود (نه صرفاً ناپایداری محیطی) چون رندر آن viewport طولانی‌تر از پنجره رقابتی race بود تا حالت خالی موقت واقعاً commit شود.

## ADR-029 — فاز ۱۳: منبع اعداد فرانما، تفکیک persist، drill-down یک‌طرفه، نمودار بدون کتابخانه

**Status:** Accepted

مانند فازهای ۱۰ تا ۱۲، Development Pack شانزده‌سندی فاز ۱۳ آغاز شد ولی مالک محصول پیش از تولید `00-README.md` درخواست پیاده‌سازی مستقیم داد؛ تنها مصنوع برنامه‌ریزی تولیدشده `docs/phase-13-situation-dashboard/PRE_IMPLEMENTATION_DEPENDENCY_REVIEW.md` است که ممیزی کدبیس و حل قطعی هر شکاف را ثبت می‌کند. پیاده‌سازی بر اساس همان سند به‌علاوه اسناد الزام‌آور موجود (`IMPLEMENTATION_PLAN.md` فاز ۱۳، `PROJECT_SPEC.md` §۱۱، `UX_MAP_AND_DESIGN_SYSTEM.md` §۵) انجام شد. تصمیم‌های حل‌شده:

1. **منبع اعداد مأموریت — مهم‌ترین تصمیم این فاز.** در کدبیس دو واژگان وضعیت *ناهم‌پوشان* وجود دارد: `persistedStatus` ستون DB (`DRAFT|SCHEDULED|CANCELLED|ARCHIVED`) و وضعیت نمایشی محاسبه‌شده `deriveMissionDisplayStatus` (`DRAFT|WAITING|IN_PROGRESS|ARRIVED|CANCELLED|ARCHIVED`) که تابع لحظه مشاهده است. یک مأموریت `SCHEDULED` بدون هیچ نوشتنی در DB و صرفاً با گذر زمان از «در انتظار حرکت» به «در حال حرکت» و سپس «رسیده» می‌رود. چون `IMPLEMENTATION_PLAN.md` صراحتاً «مأموریت‌های منتشرشده/در حال حرکت» را جدا از «مأموریت‌های پایان‌یافته» می‌خواهد، `getMissionSummary()` موجود (که فقط `prisma.count` روی `persistedStatus` است) ساختاراً قادر به پاسخ نیست. بنابراین `dashboard-service.ts` رکوردهای مأموریت را با `select` باریک (سه فیلد وضعیت + نوع خودرو) می‌خواند و همان تابع محض دامنه را رویشان اجرا می‌کند — نه یک منطق SQL موازی. این تضمین می‌کند عدد فرانما و وضعیت نقشه فاز ۱۰ هرگز واگرا نشوند. «در انتظار» صراحتاً `WAITING` تعریف شد (نه `DRAFT`)؛ پیش‌نویس شمارنده جدا دارد چون هنوز تعهد عملیاتی نیست.
2. **endpoint اختصاصی به‌جای استفاده مجدد از `*/summary`های موجود.** چهار endpoint خلاصه موجود گیت نقشی ناسازگار دارند (ناوگان فقط Admin، مأموریت/مرسوله بدون بیننده وضعیت) در حالی که `PROJECT_SPEC.md` §۴ مشاهده فرانما را به هر سه نقش می‌دهد. به‌جای تغییر گیت آن‌ها (که صفحات مدیریتی موجود را تحت تأثیر می‌گذاشت)، `GET /api/v1/dashboard/summary` با دسترسی هر سه نقش اضافه شد و endpointهای قدیمی دست‌نخورده ماندند.
3. **تفکیک persist.** *چیدمان* widgetها (ترتیب و نمایان‌بودن) در `localStorage` با کلید `armanhaml-dashboard-layout` و شماره نسخه ذخیره می‌شود — همان الگوی theme، چون ترجیح نمایشی محض است. در مقابل *بازه زمانی و به‌روزرسانی خودکار* عمداً persist نمی‌شوند و هر بار به پیش‌فرض بازمی‌گردند — همان استدلال ADR-028: اپراتور نباید بی‌صدا روی حالت کهنه نشست قبل بماند. `reconcileLayout` شناسه ناشناس را دور می‌ریزد و widget جدید نسخه بعدی را به انتها الحاق می‌کند، پس ارتقا هرگز چیدمان کاربر را نمی‌شکند.
4. **drill-down یک‌طرفه و یک‌باره.** هیچ صفحه‌ای در سامانه فیلترش را از URL نمی‌خواند. به‌جای ساخت فیلتر جدید (خارج از دامنه این فاز)، یک hook کوچک `useSearchParamSeed` اضافه شد که فقط به‌عنوان مقدار اولیه `useState` استفاده می‌شود: صفحه مقصد فیلتر *موجود* خود را از query string بذر می‌گیرد، پس از آن کاربر حاکم است و هیچ صفحه‌ای URL را بازنویسی نمی‌کند. مقدار خارج از allowlist بی‌صدا نادیده گرفته می‌شود. تفکیک مقصد بر اساس واژگان §۱ است: KPI با واژگان persisted به صفحه فهرست می‌رود، KPI با واژگان محاسباتی به `/map` که تنها سطح فهمنده `WAITING/IN_PROGRESS/ARRIVED` است.
5. **نمودار بدون dependency جدید.** هیچ کتابخانه نموداری نصب نشد؛ دونات و میله افقی به‌صورت SVG درون‌خطی ساخته شدند، هم‌راستا با ممنوعیت CDN در `CLAUDE.md` §۲ و همان مسیری که `icons.tsx` رفته است. خودِ SVG با `aria-hidden` از درخت دسترسی حذف و legend متنی (برچسب+مقدار+درصد) جایگزین واقعی آن است، پس رنگ هرگز تنها حامل معنا نیست.
6. **جابه‌جایی چیدمان با دکمه، نه drag & drop.** کشیدن‌ورهاکردن روی لمس/کیبورد/صفحه‌خوان یا کار نمی‌کند یا نیازمند کتابخانه جدید است؛ دکمه‌های «بالا/پایین» با target لمسی ۴۴px با هر سه ورودی کار می‌کنند.
7. **بدون انیمیشن شمارنده JS.** `UX_MAP_AND_DESIGN_SYSTEM.md` §۵ انیمیشن شمارنده کوتاه را مجاز می‌داند ولی هم‌زمان «ارقام همیشه قابل انتخاب/خواندن» را الزام می‌کند. عدد به‌صورت متن نهایی رندر می‌شود تا صفحه‌خوان و آزمون خودکار هرگز مقدار میانی نبینند؛ حرکت فقط به گذارهای CSS محدود شد که خودکار از `prefers-reduced-motion` موجود تبعیت می‌کنند.
8. **اثر شیشه‌ای نامتقارن بین دو theme.** در تیره `--color-panel` از قبل نیمه‌شفاف است و با `backdrop-blur` موجود Panel اثر شیشه واقعی می‌دهد؛ در روشن پنل مات می‌ماند تا کنتراست متن فارسی حفظ شود و فقط hairline نورانی باقی می‌ماند. هر دو از `--color-panel-glow` استفاده می‌کنند — تنها token از پیش تعریف‌شده‌ای که تا فاز ۱۲ هیچ مصرف‌کننده‌ای نداشت.

**refactor لازم جانبی:** `missionDisplayStatusLabel` از `src/features/missions/status-labels.ts` به `src/lib/domain/mission-labels.ts` منتقل شد و فایل قبلی آن را re-export می‌کند. علت: سرویس آمار سمت سرور برای برچسب‌گذاری قطاع‌های نمودار به آن نیاز دارد و فایل قبلی `StatTone` را از `components/ui/stat-card` وارد می‌کرد — یعنی وارد کردن آن به سرور، نوع یک component را وارد bundle سرور می‌کرد و مرزبندی لایه‌ها (`CLAUDE.md` §۲) را می‌شکست. هیچ مصرف‌کننده UI تغییر نکرد.

## ADR-030 — فاز ۱۴: کاربران، آیکن نقشه و تنظیمات سامانه

**Status:** Accepted

برخلاف فازهای ۱۰ تا ۱۳، این فاز Development Pack کامل هفده‌سندی دارد (`docs/phase-14-users-icons-settings/`) و ده ADR داخلی خودش (ADR-P14-01…10) را ثبت کرده است. این ADR فقط تصمیم‌هایی را ثبت می‌کند که *حین پیاده‌سازی* گرفته شدند یا از سند پک انحراف دارند.

1. **بدون جدول Permission — نقش‌محوری موجود دست‌نخورده ماند.** ممیزی پیش از پیاده‌سازی نشان داد سامانه از فاز ۱ یک مدل نقش‌محور ساده دارد: `RoleCode` سه‌مقداری و یک تابع `assertRole(actor, allowedRoles)` که هر ۴۰ route از آن استفاده می‌کنند. طبق قید صریح مالک محصول («هرگز یک معماری موجود را بی‌صدا جایگزین نکن» و «اگر جدول User/Role/Permission از قبل هست، تکرارش نکن»)، هیچ جدول Permission، claim یا policy engine اضافه نشد. ۱۹ endpoint جدید همان `requireActor([RoleCode.ADMIN])` را صدا می‌زنند و گیت **پیش از خواندن بدنه درخواست** اجرا می‌شود.

2. **ایمنی SVG لایه‌ای است و لایه اول، تحلیل‌گر نیست.** کنترل اصلی، *نحوه رندر* است: آیکن‌ها فقط از راه `<img src>` و از یک endpoint با `Content-Security-Policy: default-src 'none'; sandbox` سرو می‌شوند، پس حتی اگر SVG مخربی از اعتبارسنجی رد شود مرورگر اجازه اجرای script یا بارگیری منبع به آن نمی‌دهد. تحلیل‌گر allowlist در `src/lib/domain/svg-analyzer.ts` **لایه دوم** است، نه اول — چون sanitizerهای دست‌نویس SVG تاریخاً دور زده می‌شوند. هرگز `dangerouslySetInnerHTML` یا SVG درون‌خطی برای محتوای آپلودی استفاده نمی‌شود.

3. **فایل آیکن بیرون از `public/` ذخیره می‌شود.** Next.js محتوای `public/` را *پیش از* middleware و به‌صورت ایستا سرو می‌کند؛ گذاشتن آیکن‌ها آنجا یعنی دسترسی بدون احراز هویت و بدون هیچ رد ممیزی. ریشه ذخیره `ICON_STORAGE_ROOT` (پیش‌فرض `storage/icons`) است، نام فایل همیشه `{uuid}.{png|svg}` ساخته می‌شود (نام اصلی فقط برای نمایش نگه داشته می‌شود) و `resolveIconPath` علاوه بر تطبیق الگو، مهار مسیر را با `realpath` بررسی می‌کند.

4. **تنظیمات، رجیستری کد دارند نه ستون.** `SystemSetting` فقط `key` (کلید اصلی) و `value` (JSON) است؛ نوع، بازه، مقدار پیش‌فرض، برچسب فارسی و اتصال به متغیر محیطی همه در `src/lib/settings/settings-registry.ts` هستند. نتیجه: افزودن یک تنظیم جدید هیچ migration نمی‌خواهد. رکورد **تنبل** ساخته می‌شود، پس جدول دقیقاً «انحراف‌های اپراتور از پیش‌فرض» است. ترتیب اولویت: متغیر محیطی ← رکورد DB ← پیش‌فرض رجیستری؛ متغیر محیطی همیشه برنده است و تنظیم را قفل می‌کند (زیرساخت نباید از یک فرم وب قابل بازنویسی باشد). `assertRegistryIsSound` کلیدهای شبیه‌راز را رد می‌کند تا هیچ اعتبارنامه‌ای در تنظیمات معمولی ذخیره نشود؛ اعتبارنامه‌ها همان الگوی موجود `MapProvider.secretReference` را دنبال می‌کنند.

5. **قفل «آخرین مدیر» باید *پیش از* نوشتن گرفته شود و مرتب باشد — کشف حین آزمون همروندی.** نسخه اول، ابتدا ردیف هدف را به‌روزرسانی می‌کرد و سپس با `SELECT … FOR UPDATE` مجموعه مدیران فعال را قفل می‌کرد. دو مشکل واقعی در آزمون آشکار شد:
   - PostgreSQL اجازه `FOR UPDATE` همراه تابع تجمیعی نمی‌دهد؛ `count(*)` با خطای `0A000` می‌شکست. شمارش به سمت JS منتقل شد.
   - با قفل‌گذاری *پس از* به‌روزرسانی، دو تراکنش موازی که هرکدام مدیر دیگری را غیرفعال می‌کنند هرکدام یک ردیف در دست دارند و منتظر دیگری می‌مانند — بن‌بست کلاسیک که به‌جای ۴۰۹ خطای ۵۰۰ می‌داد. اکنون `lockActiveAdmins` در **ابتدای** تراکنش و با `ORDER BY u.id` اجرا می‌شود تا همه تراکنش‌ها قفل‌ها را به یک ترتیب بگیرند؛ ارزیابی قاعده همچنان *پس از* اعمال تغییر انجام می‌شود تا شمارش وضعیت نهایی را ببیند.

6. **تعارض ساخت هم‌زمان یک تنظیم = تعارض نسخه، نه خطای سرور.** دو نویسنده موازی روی کلیدی که هنوز رکورد ندارد هر دو به شاخه `create` می‌رسند و بازنده نقض کلید یکتا می‌گیرد. این دقیقاً همان معنای تعارض نسخه است (نسخه صفرِ فرستنده کهنه شده) و به `SETTING_VERSION_CONFLICT` با وضعیت ۴۰۹ نگاشت شد.

7. **سهمیه نوشتن مدیریتی (SEC-16) جدا از شمارنده ورود.** محدودکننده نرخ موجود فقط تلاش‌های *ناموفق* ورود را می‌شمرد. یک شمارنده مستقل `consumeQuota` اضافه شد که *هر* نوشتن مدیریتی را می‌شمارد (۱۲۰ در دقیقه به‌ازای هر actor) و کلیدش شناسه کاربر است نه IP — هدف، مهار یک حساب مدیریتی ربوده‌شده است نه یک شبکه مشترک. مثل شمارنده ورود، per-process است و برای استقرار چندنمونه‌ای باید به store مشترک منتقل شود.

8. **آیکن سفارشی روی نقشه با bearing نمی‌چرخد.** نشانگر پیش‌فرض یک فلش است و چرخاندنش معنا دارد؛ جهت ذاتی یک تصویر آپلودی معلوم نیست و چرخاندن آن خودروها را وارونه نشان می‌دهد. وضعیت مأموریت به‌جای رنگ دایره، در حلقه رنگی دور آیکن دیده می‌شود. نبود آیکن، آیکن حذف‌شده/غیرفعال و خطای بارگذاری تصویر همگی **بی‌صدا** به همان نشانگر دایره‌ای برمی‌گردند؛ هرگز تصویر شکسته نمایش داده نمی‌شود.

9. **انحراف عمدی از §۴٫۶ سند API — تخصیص آیکن توکن نسخه نمی‌گیرد.** سند برای `PUT /api/v1/icons/assignments` یک `version` از *موجودیت مقصد* می‌خواست، ولی `OrganizationUnit`، `VehicleType` و `Vehicle` اصلاً ستون `version` ندارند و افزودن آن به سه موجودیت، یک migration خارج از دامنه این فاز است. تخصیص یک نوشتن تک‌فیلدی است، کاملاً audit می‌شود و آخرین نوشتن برنده است. `SystemSetting`، `IconAsset` و `User` توکن نسخه دارند و همه مسیرهای پرمخاطره‌شان همروندی خوش‌بینانه کامل دارند.

10. **E-04 عمداً سناریوی سراسری «آخرین مدیر» را در e2e بازسازی نمی‌کند.** رسیدن به آن حالت یعنی غیرفعال‌کردن همه مدیران فعال سامانه؛ روی DB مشترکی که چهار پروژه viewport به‌موازات از آن استفاده می‌کنند، این کار ورود همه اجراهای دیگر را می‌شکند و اگر تست وسط کار خطا بخورد خط پایه محیط خراب می‌ماند (این دقیقاً یک‌بار رخ داد). مرز قاعده در `tests/unit/user-rules.test.ts` و همروندی آن در CX-02 پوشش داده می‌شود؛ e2e فقط اثبات می‌کند گیت روی مسیر HTTP نشسته است. در همین راستا `tests/e2e/seed-users.ts` اصلاح شد تا ستون‌های وضعیت جدید فاز ۱۴ را هم بازنشانی کند و خط پایه قطعی بماند.

## ADR-031 — فاز ۱۵: تکمیل چرخه عمر مأموریت

**Status:** Accepted

فاز ۱۵ Development Pack کامل هفده‌سندی دارد (`docs/phase-15-mission-lifecycle-completion/`) با ده ADR داخلی (ADR-P15-01…10) که همگی پیش از پیاده‌سازی **Accepted** بودند. آن‌ها اینجا تکرار نمی‌شوند؛ این ADR فقط تصمیم‌هایی را ثبت می‌کند که *حین پیاده‌سازی* گرفته شدند یا از سند انحراف دارند.

1. **گارد گذار درون تراکنش و روی داده تازه ارزیابی می‌شود، نه صرفاً با تکیه بر توکن نسخه.** نسخه اول، مأموریت را پیش از تراکنش می‌خواند و همان‌جا گذار را تأیید می‌کرد؛ استدلال این بود که توکن همروندی هر تغییر هم‌زمانی را می‌گیرد. این استدلال یک وابستگی پنهان دارد: تنها در صورتی درست است که **هر** مسیر نوشتنی نسخه را بالا ببرد. `publishMission` این کار را نمی‌کرد. به‌جای تکیه بر قاعده‌ای که یک تابع فراموش‌کار می‌تواند بی‌صدا بشکند، `loadMissionInTx` اضافه شد و گارد درون تراکنش تکرار می‌شود (TX-02). مستقلاً `commitMissionAssignment` هم اکنون نسخه را افزایش می‌دهد (CC-01).

2. **`updateMission` روی مسیر عملیاتی با خواندنِ قفل‌دار بررسی می‌شود، نه با نوشتن مشروط.** `commitMissionAssignment` خودش در انتها نسخه را یک واحد بالا می‌برد؛ یک `updateMany` مشروط اضافه در ابتدای همان تراکنش، نسخه را دو واحد جابه‌جا می‌کرد و CC-01 («دقیقاً یک واحد») را می‌شکست. به‌جای آن یک `SELECT … FOR UPDATE` روی همان ردیف انجام و نسخه مقایسه می‌شود؛ قفل تا commit نگه داشته می‌شود، پس پنجره رقابتی وجود ندارد.

3. **`ApiError` سمت کلاینت اکنون `code` را نگه می‌دارد.** پیش از این فقط پیام را نگه می‌داشت. CC-06 می‌خواهد تعارض نسخه *رفتار* متفاوتی داشته باشد (نمایش دکمه «تازه‌سازی»)، نه فقط متن متفاوت؛ و `SHIPMENT_ALREADY_ASSIGNED` توضیح دیگری می‌خواهد. تطبیق روی متن پیام شکننده بود و با هر ویرایش ترجمه می‌شکست.

4. **`COMPLETED`/`FAILED` به گزینه‌های پالایه نقشه عملیاتی افزوده *نشدند*.** صحنه نقشه فقط مأموریت‌های `SCHEDULED` را می‌خواند، پس این دو مقدار هرگز در آن فهرست ظاهر نمی‌شوند و افزودن گزینه‌ای که همیشه صفر نتیجه می‌دهد فقط گمراه‌کننده است. تفکیک این دو در فهرست مأموریت‌ها و فرانما دیده می‌شود، جایی که مأموریت‌های پایان‌یافته واقعاً حضور دارند.

5. **قید زمانی واقعی که از ترکیب دو فاز پدید می‌آید و باید مستند بماند:** فاز ۷ زمان شروع را الزاماً در آینده می‌خواهد و فاز ۱۵ زمان رسیدن واقعی را الزاماً در گذشته (LR-01) و پس از زمان شروع (LR-02). نتیجه: **یک مأموریت تنها پس از فرارسیدن زمان شروع برنامه‌ریزی‌شده‌اش قابل تکمیل است.** این محدودیت نیست، رفتار درست است — نمی‌توان برای مأموریتی که هنوز شروع نشده رسیدن ثبت کرد — ولی چون در هیچ سند فاز صریح نبود، اینجا ثبت می‌شود تا بعداً به‌عنوان باگ گزارش نشود.

6. **`Mission.notes` قدیمی و رشته یادداشت تازه، هر دو در صفحه جزئیات دیده می‌شوند** (طبق ADR-P15-09) با برچسب متفاوت. جذب یادداشت قدیمی در رشته تازه یعنی ساختن نویسنده و زمانی که وجود ندارد — جعل داده ممیزی، حتی اگر جعل راحتی باشد.
