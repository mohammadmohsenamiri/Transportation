...
import { createRouteNewVersion } from "@/server/services/route-service";
import { routeNewVersionSchema } from "@/lib/validation/route";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = routeNewVersionSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const created = await createRouteNewVersion(id, parsed.data, result.actor);
