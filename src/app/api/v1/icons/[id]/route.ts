...
/** حذف نرم — تخصیص‌ها عمداً حفظ می‌شوند تا بازگردانی آن‌ها را برگرداند (BR-I02). */
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    return NextResponse.json(await setIconDeleted(id, true, parsed.data.version, result.actor));
  } catch (error) {
