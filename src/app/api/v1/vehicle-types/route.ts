...
import { listVehicleTypes, createVehicleType } from "@/server/services/vehicle-type-service";
import { vehicleTypeCreateSchema } from "@/lib/validation/vehicle";
...
export async function GET(request: NextRequest) {
  ...
  const items = await listVehicleTypes({ q });
...
export async function POST(request: NextRequest) {
  ...
  const parsed = vehicleTypeCreateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const created = await createVehicleType(parsed.data, result.actor);
